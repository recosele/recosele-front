...
const LoginForm: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      if (values.email === 'yan53074.work@gmail.com' && values.password === '123456') {
        const remainingMilliseconds = 24 * 60 * 60 * 1000;
        const expiryDate = new Date(new Date().getTime() + remainingMilliseconds);
        setUserInfo({ expiryDate });
        setLoading(false);
    ...
    <div className={styles.loginLayout}>
      <Form form={form} onFinish={onFinish} className={styles.form}>
        <div className={styles.login}>
          <div className={styles.title}>RECOSELE</div>
          ...
          >
            <Input prefix={<MailOutlined />} placeholder="メール" className={styles.input} />
          </Form.Item>
          <Form.Item
            name="password"
            rules={[{ required: true, message: <Fragment /> }]}
